...
class mac_ifconfig(common.AbstractMacCommand):
    ...
    def calculate(self):
        ...
        while ifnet:
            ...
            yield (name, unit, mac, ips)
            ifnet = ifnet.if_link.tqe_next
 
    def render_text(self, outfd, data):
        self.table_header(outfd, [("Interface", "10"), ("IP Address", "32"), ("Mac Address", "")])

        for (name, unit, mac, ips) in data:
            if ips:
                for ip in ips:
                    self.table_row(outfd, "{0}{1}".format(name, unit), ip, mac)
            else:
                # an interface with no IPs
                self.table_row(outfd, "{0}{1}".format(name, unit), "", mac)
